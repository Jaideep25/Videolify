...
const avatarImg = "../images/sphere.png";
...
const chunkSize = 1024 * 220; // .2mb/s
// video URL player
let videoUrlCont;
...
function setButtonsToolTip() {
  ...
  setTippy(chatRoomBtn, "Open Chats", "right-start");
  setTippy(captionBtn, "Open Captions", "right-start");
  ...
  setTippy(
    switchPushToTalk,
    "If Active, When SpaceBar is pressed the microphone will be activated, on keyup will be deactivated, like a walkie-talkie.",
    "right"
  );
  ...
  // whiteboard btns
  setTippy(wbDrawingColorEl, "Pen color", "bottom");
  setTippy(whiteboardGhostButton, "Toggle transparent background", "bottom");
  setTippy(wbBackgroundColorEl, "BG color", "bottom");
  setTippy(whiteboardPencilBtn, "Pen", "bottom");
  setTippy(whiteboardObjectBtn, "Sniper", "bottom");
  ...
  setTippy(whiteboardTriangleBtn, "Insert triangle", "bottom");
  setTippy(whiteboardCircleBtn, "Insert circle", "bottom");
  setTippy(whiteboardSaveBtn, "Download the board", "bottom");
  setTippy(whiteboardEraserBtn, "Remove object", "bottom");
  setTippy(whiteboardCleanBtn, "Clear the board", "bottom");
  ...
  setTippy(msgerVideoUrlBtn, "Share a video/audio to all participants", "top");
}
...
function initClientPeer() {
  if (!isWebRTCSupported) {
    return userLog("error", "This browser doesn't support WebRTC!");
  }
...
async function whoAreYou() {
  ...
  setTippy(initAudioBtn, "Pause the audio", "top");
  setTippy(initVideoBtn, "Pause the video", "top");
}
...
async function whoAreYouJoin() {
  if (isMobileDevice && myVideoStatus && myAudioStatus)
    await refreshLocalMedia();
  ...
  switch (videolifyTheme) {
    case "neon":
      ...
      document.documentElement.style.setProperty(
        "--msger-bg",
        "linear-gradient(to right,#380036, #0CBABA)"
      );
      ...
      document.documentElement.style.setProperty("--wb-bg", "#FFF");
...
function addChild(device, els) {
  ...
  els.forEach((el) => {
    ...
    switch (kind) {
      ...
      case "audioinput":
        option.innerHTML =
          `🎤 ` + device.label || `🎤 microphone ${el.length + 1}`;
        break;
      case "audiooutput":
        option.innerHTML =
          `🔈 ` + device.label || `🔈 speaker ${el.length + 1}`;
        break;
      default:
        break;
    }
...
async function setupLocalMedia() {
  ...
  const videoConstraints = useVideo
    ? await getVideoConstraints("default")
    : false;
...
function handleVideoZoomInOut(
  ...
  zoomIn.addEventListener("click", () => {
    if (isVideoOf(id))
      return userLog("toast", "Zoom in works only when video is on");
    if (isVideoPrivacyMode(video))
      return userLog("toast", "Can't zoom if video on privacy mode");
    zoom = zoom + 0.1;
    setTransform();
  });
  ...
  zoomOut.addEventListener("click", () => {
    if (isVideoOf(id))
      return userLog("toast", "Zoom out works only when video is on");
    if (isVideoPrivacyMode(video))
      return userLog("toast", "Can't zoom if video on privacy mode");
    zoom = zoom - 0.1;
    setTransform();
...
function handleVideoToImg(videoStream, videoToImgBtn, peer_id = null) {
  ...
  videoBtn.addEventListener("click", () => {
    if (video.classList.contains("videoCircle")) {
      return userLog("toast", "Can't take snapshot, privacy mode is ON");
    }
    if (peer_id !== null) {
    ...
    userLog("toast", "Can't take a snapshot, video is disabled");
  });
}
...
function setMyWhiteboardBtn() {
  ...
  whiteboardGhostButton.addEventListener("click", (e) => {
    ...
    setWhiteboardBgColor(
      wbIsBgTransparent ? "rgba(0, 0, 0, 0.100)" : wbBackgroundColorEl.value
    );
    //wbIsBgTransparent ? wbCanvasBackgroundColor('rgba(0, 0, 0, 0.100)'): setTheme(videolifyTheme);
  });
}
...
function setMySettingsBtn() {
  ...
  // Sounds
  userLog("toast", "Notifications ~ " + (notifyBySound ? "ON" : "OFF"));
  playSound("switch");
...
function getAudioVideoConstraints() {
  ...
  if (audioConstraints) {
    ...
    audioConstraints["deviceId"] = audioSource
      ? { exact: audioSource }
      : undefined;
  }
  return {
...
function shareRoomMeetingURL(checkScreen = false) {
  ...
  Swal.fire({
    ...
    html: `
        ...
        <p style="background:transparent; color:white;">No need for apps, simply capture the QR code with your mobile camera Or Invite someone else to join by sending them this URL</p>
...
function handleAudio(e, init, force = null) {
  ...
  if (init) {
    ...
    setTippy(
      initAudioBtn,
      myAudioStatus ? "Pause the audio" : "Start the audio",
      "top"
    );
...
function handleVideo(e, init, force = null) {
  ...
  if (init) {
    initVideoBtn.className = myVideoStatus
      ? className.videoOn
      : className.videoOff;
    setTippy(
      initVideoBtn,
      myVideoStatus ? "Pause the video" : "Start the video",
      "top"
    );
    ...
    if (videoSender) {
      ...
      stream.getTracks().forEach((track) => {
        if (track.kind === "video") {
          ...
          console.log("ADD VIDEO TRACK TO", {
            peer_id: peer_id,
            peer_name: peer_name,
          });
        }
  ...
  try {
    ...
    userLog(
      ...
      `<div style="text-align: left;">
                ...
                <br/>
                Please wait for the video to be processed, then will be downloaded to your ${currentDevice} device.
            </div>`
    );
...
function showChatRoomDraggable() {
  ...
  setTippy(chatRoomBtn, "Close Chats", "right-start");
}
...
function showCaptionDraggable() {
  ...
  setTippy(captionBtn, "Close Captions", "right-start");
}
...
function checkMsg(text) {
  ...
  if (isValidHttpURL(text)) {
    ...
    return (
      '<a href="' + text + '" target="_blank" class="msg-a">' + text + "</a>"
    );
  }
...
function setMyAudioStatus(status) {
  ...
  setTippy(
    audioBtn,
    status ? "Pause the audio" : "Start the audio",
    "right-start"
  );
...
function setMyVideoStatus(status) {
  ...
  if (!isMobileDevice) {
    ...
    setTippy(
      videoBtn,
      status ? "Pause the video" : "Start the video",
      "right-start"
    );
  }
  status ? playSound("on") : playSound("off");
...
function setPeerAudioStatus(peer_id, status) {
  ...
  if (peerAudioStatus) {
    ...
    setTippy(
      peerAudioStatus,
      status ? "Peer is unmuted" : "Peer is muted",
      "bottom"
    );
    status ? playSound("on") : playSound("off");
  }
...
function setPeerVideoStatus(peer_id, status) {
  ...
  if (peerVideoStatus) {
    ...
    setTippy(
      peerVideoStatus,
      status ? "Peer video is on" : "Peer video is off",
      "bottom"
    );
    status ? playSound("on") : playSound("off");
  }
...
function handleScreenStart(peer_id) {
  ...
  if (remoteVideoStatusBtn) {
    ...
    setTippy(remoteVideoStatusBtn, "Peer is screen sharing", "bottom");
  }
...
function handleScreenStop(peer_id, peer_use_video) {
  ...
  if (remoteVideoStatusBtn) {
    ...
    setTippy(remoteVideoStatusBtn, "Peer isn't screen sharing", "bottom");
  }
  if (remoteVideoStream) {
...
function setupWhiteboardCanvas() {
  ...
  wbCanvas.freeDrawingBrush.color = "#000";
...
function sendFileData(peer_id, broadcast) {
  ...
  const readSlice = (o) => {
    for (let peer_id in fileDataChannels) {
      ...
      if (
        fileDataChannels[peer_id].bufferedAmount >
        fileDataChannels[peer_id].bufferedAmountLowThreshold
      ) {
